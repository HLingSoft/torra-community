...
onMounted(async () => {
  ...
  if (inputRef.value && !node.data.saved) {
    ...
    addInputPort(props.id!, currentNode.value.data!.inputVariable.id, 'aquamarine', inputRef.value.offsetTop -10)
  }

    if (filterKeyRef.value && !node.data.saved) {
        currentNode.value.data!.filterKey.id = nanoLowercaseAlphanumericId(10)
        addInputPort(props.id!, currentNode.value.data!.filterKey.id, 'aquamarine', filterKeyRef.value.offsetTop + filterKeyRef.value.clientHeight / 2)
    }
      ...
      <CardContent class="text-white space-y-8 -mt-8 flex-1">
        ...
        <div ref="filterKeyRef">
          ...
          <div class="w-full mt-5">
            <EditTextDialog class="w-full" :disabled="currentNode.data.filterKey.connected" :model-value="currentNode.data.filterKey.value || ''" placeholder="请输入字段名称" @save="(val) => currentNode!.data!.filterKey.value = val" />
          </div>
        </div>
      </CardContent>
