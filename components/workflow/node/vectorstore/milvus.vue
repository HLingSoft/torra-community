...
onMounted(async () => {
  ...
  if(partitionValueRef.value && !data.saved) {
    data.partitionValueVariable.id = nanoLowercaseAlphanumericId(10)
    addInputPort(id, data.partitionValueVariable.id, 'aquamarine', partitionValueRef.value.offsetTop + partitionValueRef.value.clientHeight / 2)
  }
  ...
  if(embeddingRef.value && !data.saved) {
    ...
    addInputPort(id, data.embeddingVariable.id, 'yellow', embeddingRef.value.offsetTop + embeddingRef.value.clientHeight / 2-16)
  }
  if(ingestDataRef.value && !data.saved) {
    data.ingestDataVariable.id = nanoLowercaseAlphanumericId(10)
    addInputPort(id, data.ingestDataVariable.id, 'yellow', ingestDataRef.value.offsetTop + ingestDataRef.value.clientHeight / 2-16)
  }
...
watch(edges, () => {
  ...
  data.partitionValueVariable.connected = edges.value.some(edge => edge.target === data.partitionValueVariable.id)
  ...
  <Card v-if="currentNode && currentNode.data" class="!pb-0 w-96 text-white bg-background  rounded-lg  flex flex-col focus:outline-none  focus:shadow-lg focus:shadow-card   focus:border focus: border-card">
    ...
    <CardContent class="text-white space-y-8 -mt-8  flex-1 nodrag nopan cursor-auto ">
      ...
      <div ref="partitionValueRef">
        ...
        <div class="w-full  mt-5">
          <EditTextDialog class="w-full" :disabled="currentNode.data.partitionValueVariable.connected" :model-value="currentNode.data.partitionValueVariable.value || ''" placeholder="Typing something" @save="(val) => currentNode!.data!.partitionValueVariable.value = val" />
        </div>
      </div>
