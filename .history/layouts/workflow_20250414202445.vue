...
const playWorkflow = async () => {
  ...
  workflowAV.value!.nodes.forEach((node) => {
    node.data = {
      ...node.data,
      saved: true,
    }
  ...
  workflowAV.value!.edges.forEach((edge) => {
    edge.data = {
      ...edge.data,
      saved: true,
    }
  ...
  // const { nodes: workflowNodes, edges: workflowEdges } = workflow.value
  // const nodeText = workflowNodes.map(n => `Node: [${n.id}] type: ${n.type}`).join('\n')
  // const edgeText = workflowEdges.map(e => `Edge: ${e.source} -> ${e.target}`).join('\n')

  // const plainText = `${nodeText}\n${edgeText}`

  // console.log(`🔹 Workflow (Plain Text):\n${plainText}`)
  playgroundOpen.value = true
  // const result = await runWorkflow(workflow.value)
        ...
        <SidebarInset>
          <header class=" flex fixed z-50 w-full top-0  bg-[#18181B] h-16 shrink-0 items-center gap-2 border-b border-[hsl(var(--border))] px-4">
            ...
            <div class="flex-shrink-0 ml-4">
              <Button variant="ghost" class="hidden md:flex" @click="playWorkflow">
                <div class="flex items-center space-x-2">
