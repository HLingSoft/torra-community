...
<template>
  <div class="dark  ">
    <ClientOnly>
      <SidebarProvider>
        <WorkflowSidebar  @open="isCollapsed=false"  @close="isCollapsed=true" />
        <SidebarInset>
          <header :class="{'pr-72':!isCollapsed}" class=" transition-all duration-300 flex fixed z-50 w-full  top-0  bg-[#18181B] h-16 shrink-0 items-center gap-2 border-b border-[hsl(var(--border))] px-4">
            ...
            <div>
              ...
              <DropdownMenu v-if="user">
                  <DropdownMenuTrigger as-child @click.stop>
                    <Button variant="secondary" size="icon" class="rounded-full">
                      <div class="avatar online ">
                        <div class="w-8 rounded-full ring ring-offset-2 ring-white ring-offset-base-100">
                          <img :src="user.avatar" />
                        </div>
                      </div>
                  ...
                  <DropdownMenuContent class="w-56 space-y-2 dark" align="end">
