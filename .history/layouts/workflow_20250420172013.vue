      ...
      <SidebarProvider>
        ...
        <SidebarInset>
          <header class=" flex fixed z-50 w-full py-5  top-0  bg-[#18181B] h-16 shrink-0 items-center gap-2 border-b border-[hsl(var(--border))] px-4">
            ...
            <Separator orientation="vertical" class="mr-2 h-3  dark " />
  ...
  <DropdownMenu>
        <DropdownMenuTrigger as-child>
          <Button
            size="sm"
            variant="outline"
            class="w-full text-white bg-[#18181B] rounded-lg group flex flex-col focus:outline-none focus:shadow-lg focus:shadow-[#000000]  focus:border focus:border-[#27272A]"
          >
        ...
        >
          <DropdownMenuItem
            ...
            @select="currentWorkflow = workflow"
          >
            {{ workflow.name }}
            <Check v-if="workflow.objectId === currentWorkflow?.objectId" class="ml-auto" />
                ...
                <!-- <Select>
    <SelectTrigger>
      <SelectValue  :value="currentWorkflow?.name|| ''" />
    </SelectTrigger>
    <SelectContent class="w-56 dark max-h-56">
      <SelectGroup>

        <SelectItem :value="workflow.objectId" v-for="workflow in allWorkflows" :key="workflow.objectId" @click="currentWorkflow=workflow">
          {{ workflow.name }}
