...
console.log('json',json)
 ...
 // 你的起点 node id，比如 "jem27m8cc7" 是 ChatInput
const startNodeId = 'jem27m8cc7';

const sortedIds = topologicalSortFromStart(json.nodes, json.edges, startNodeId);
// console.log('[DAG] 拓扑排序结果:', result);
...
function getConnectedNodes(startId, edges) {
...
function topologicalSortFromStart(nodesMap, edges, startNodeId) {
  // 第一步：找出从起点出发能走到的所有节点
  const connected = getConnectedNodes(startNodeId, edges);

  // 第二步：构建图结构，只包括连通子图里的节点
  const graph = {};
  const inDegree = {};
  ...
  for (const edge of edges) {
    if (connected.includes(edge.source) && connected.includes(edge.target)) {
      graph[edge.source].push(edge.target);
      inDegree[edge.target]++;
    }
  }

  // 第三步：拓扑排序（Kahn's algorithm）
  const queue = connected.filter((id) => inDegree[id] === 0);
  const sorted = [];
