...
if (node.data.type === 'IfElse') {
  ...
  const fullActiveSet = new Set([...activeSet])
  const upstream = collectUpstream([...activeSet], inputConnections)
  upstream.forEach(id => fullActiveSet.add(id))
  
  const toAdd = [...fullActiveSet].filter(id => !executed.has(id) && !prunedPending.includes(id))
