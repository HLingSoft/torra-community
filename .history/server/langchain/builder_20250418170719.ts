...
console.log('json',json)
  pruneDisconnectedCustomNodes(json)
    ...
    // 只取从输入节点出发可达的 custom 节点
    const reachableCustomIds = getReachableCustomNodeIds(json)
    console.log('[DAG] 可达的 custom 节点:', reachableCustomIds)
...
/** 找出从所有起点(如 ChatInput)出发可达的 custom 节点 */
function getReachableCustomNodeIds(json: LangFlowJson): string[] {
  const startPorts = Object.values(json.nodes)
    .filter(n => isStartNode(json, n.id))
    .map(n => n.id)
  const visited = new Set<string>()
  const queue = [...startPorts]
  ...
  while (queue.length) {
    ...
    for (const edge of json.edges) {
      if (edge.source === cur) {
        const parent = edge.data.targetParent
