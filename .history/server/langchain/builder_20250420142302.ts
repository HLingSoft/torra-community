...
export async function executeDAG(
  ...
  while (pending.length > 0) {
    ...
    // }
    for (const [inputPortId, conns] of Object.entries(connMapForNode)) {
      if (Array.isArray(conns)) {
        resolvedInput[inputPortId] = conns
          .map(conn => results[conn.fromNodeId]?.[conn.fromPortId])
          .filter(v => v !== undefined)
      } else {
        resolvedInput[inputPortId] = results[conns.fromNodeId]?.[conns.fromPortId]
      }
    }
