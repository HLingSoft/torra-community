...
function pruneDisconnectedCustomNodes(json: LangFlowJson): void {
  ...
  // Step 1: 所有出现在 edge.source 或 edge.target 中的节点，都标记为“连接中”
  for (const edge of json.edges) {
    connectedNodeIds.add(edge.source)
    connectedNodeIds.add(edge.target)
  }

  // Step 2: 扫描所有 custom 节点
  const nodesToDelete = new Set<string>()
  for (const [nodeId, node] of Object.entries(json.nodes)) {
    if (node.type === 'custom') {
      const isConnected = connectedNodeIds.has(nodeId)
      const childIds = Object.entries(json.nodes)
        .filter(([_, childNode]) => childNode.data?.parentNode === nodeId)
        .map(([childId]) => childId)

      const childrenConnected = childIds.some(id => connectedNodeIds.has(id))

      if (!isConnected && !childrenConnected) {
        nodesToDelete.add(nodeId)
        childIds.forEach(childId => nodesToDelete.add(childId))
      }
    }
  }
