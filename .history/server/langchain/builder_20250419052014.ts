...
export async function executeDAG(
  ...
  while (i < sortedIds.length) {
    ...
    if (node.data.type === 'IfElse') {
      ...
      const skipPortName = isTrueBranch
        ? node.data.falseOutputVariable.name
        : node.data.trueOutputVariable.name

      const activePortName = isTrueBranch
        ? node.data.trueOutputVariable.name
        : node.data.falseOutputVariable.name
...
const skipPortNodeId = findPortNodeIdByName(json, node.id, skipPortName)
const activePortNodeId = findPortNodeIdByName(json, node.id, activePortName)
...
for (const conn of outputConnections[node.id]?.[skipPortNodeId] || [] as OutputConnection[]) {
...
const entryNodeIds =
  outputConnections[node.id]?.[activePortNodeId]?.map(conn => conn.toNodeId) || []
      ...
      sortedIds = [...sortedIds.slice(0, i + 1), ...extraNodes]
...
// 🔍 从 variable name 找出对应的 port 节点 ID
function findPortNodeIdByName(
  json: LangFlowJson,
  parentNodeId: string,
  portName: string,
): string | undefined {
  return Object.values(json.nodes).find(
    (n) =>
      n.data?.parentNode === parentNodeId &&
      n.data?.key === portName,
  )?.id
}
