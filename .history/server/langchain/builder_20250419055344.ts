...
export async function executeDAG(
  ...
  while (pending.length > 0) {
    ...
    if (node.data.type === 'IfElse') {
      ...
      const skipPortId = isTrueBranch ? node.data.falseOutputVariable.id : node.data.trueOutputVariable.id
      const activePortId = isTrueBranch ? node.data.trueOutputVariable.id : node.data.falseOutputVariable.id
      ...
      const entryNodeIds = outputConnections[node.id]?.[activePortNodeId]?.map(conn => conn.toNodeId) || []
      ...
      const toRemove = [...rawToRemove].filter(id => !activeBranchSet.has(id))
      pending = pending.filter(id => !toRemove.includes(id))

      const branchToExecute = [...activeBranchSet].filter(id => !executed.has(id) && !pending.includes(id))
      const extraNodes = topoSort(json.nodes, buildInputConnections(json), branchToExecute)
      pending.unshift(...extraNodes)
    }
  }

  const lastNodeId = [...executed].at(-1)!
