...
export async function executeDAG(
  ...
  for (let i = 0; i < sortedIds.length; i++) {
    ...
    if(node.id==='trW_DgK_XGrcmLrg_K674'){
      console.log("results",results)
      console.log(`✅ 执行节点 [${node.id}](${node.data.type}) 得到结果:`, resolvedInput)
    }
    ...
    console.log(`🔗 [${node.id}](${node.data.type}) 变量:`, resolvedInput)
    ...
    // console.log(`✅ 执行节点 [${node.id}](${node.data.type}) 耗时: ${formatElapsed(elapsed)}`)
    if(node.id==='trW_DgK_XGrcmLrg_K674'){
