...
export async function executeDAG(
  ...
  for (let i = 0; i < sortedIds.length; i++) {
    ...
    if (node.data.type === 'IfElse') {
      ...
      // ✅ 重新拓扑排序，把 false 分支后续流程也排进来
      const remainingCustomNodes = sortedIds.slice(i + 1) // i 是当前节点 index
      const rest = topoSort(json.nodes, buildInputConnections(json), remainingCustomNodes)
      sortedIds = sortedIds.slice(0, i + 1).concat(rest)
