...
//   }
// }
function pruneDisconnectedCustomNodes(json: LangFlowJson, entryNodeType = 'chat_input') {
  ...
  // 找所有入口节点（类型为 chat_input，可根据你定义的类型名称调整）
  const entryNodes = Object.entries(json.nodes)
    .filter(([_, node]) => node.type === entryNodeType)
    .map(([nodeId]) => nodeId)

  // 从入口节点开始 BFS 遍历所有可达节点
