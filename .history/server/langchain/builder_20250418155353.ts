...
function pruneDisconnectedCustomNodes(json: LangFlowJson, entryType = 'ChatInput') {
  ...
  // 5. 只保留 visited 中 type === 'custom' 的节点
  const customToKeep = new Set(
    [...visited].filter(id => json.nodes[id]?.type === 'custom')
  )

  // 6. 删除所有不是 custom 或 不在 reachable custom 集合中的节点
  for (const nodeId of Object.keys(json.nodes)) {
    const node = json.nodes[nodeId]
    if (node.type !== 'custom' || !customToKeep.has(nodeId)) {
      delete json.nodes[nodeId]
    }
  }

  // 7. 清理无用 edges（只保留连接的两个端都属于 customToKeep 的）
  json.edges = json.edges.filter(edge => {
    ...
    return (
      fromNode &&
      toNode &&
      customToKeep.has(fromNode) &&
      customToKeep.has(toNode)
    )
  })

  console.log(`[prune] ✅ 保留 ${customToKeep.size} 个 custom 节点`)
}
