...
export async function executeDAG(
  ...
  while (queue.length > 0) {
    ...
    const node = json.nodes[nodeId]
    const connMap = inputConnections[node.id] || {}
    const resolvedInput: Record<string, any> = {}
    ...
    // 所有输入未就绪，先跳过，等待其他上游节点执行完后重新入队
    if (Object.keys(connMap).length > 0 && !allInputsReady) {
    ...
    // 推入所有 output 有值的下游节点（延迟执行，等输入都准备好）
    for (const conn of outputConnections[nodeId] || []) {
      const outVal = output[conn.fromPortId]
      if (outVal !== undefined && !executed.has(conn.toNodeId)) {
        queue.push(conn.toNodeId)
        pending.add(conn.toNodeId)
      }
    }
  }
