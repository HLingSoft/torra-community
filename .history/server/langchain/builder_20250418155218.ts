...
function pruneDisconnectedCustomNodes(json: LangFlowJson, entryType = 'ChatInput') {
  ...
  // 1. 映射 portId → nodeId（用于 edge.source/target → node）
  const portIdToNodeId = new Map<string, string>()
  for (const node of Object.values(json.nodes)) {
    if (!node.data) continue
    const allVars = [
      node.data.languageModelVariable,
      node.data.inputMessageVariable,
      node.data.structuredOutputVariable,
      node.data.dataFrameOutputVariable,
      ...(node.data.variables || []),
    ].filter(Boolean)
    for (const variable of allVars) {
      portIdToNodeId.set(variable.id, node.id)
    }
  }

  // 2. 构建 node 邻接表（通过 edge 溯源）
  const nodeAdjacency: Record<string, Set<string>> = {}
  for (const edge of json.edges) {
    const sourceNodeId = portIdToNodeId.get(edge.source)
    const targetNodeId = portIdToNodeId.get(edge.target)
    if (sourceNodeId && targetNodeId) {
      if (!nodeAdjacency[sourceNodeId]) nodeAdjacency[sourceNodeId] = new Set()
      nodeAdjacency[sourceNodeId].add(targetNodeId)
    }
  }

  // 3. 找入口节点（根据 type）
  const entryIds = Object.entries(json.nodes)
    .filter(([_, node]) => node.type === entryType)
    .map(([id]) => id)

  if (entryIds.length === 0) {
    console.warn('[prune] No entry node found')
    return
  }
  ...
  while (queue.length) {
    ...
    const neighbors = nodeAdjacency[current] || []
  ...
  json.edges = json.edges.filter(edge => {
    const s = portIdToNodeId.get(edge.source)
    const t = portIdToNodeId.get(edge.target)
    return s && t && reachable.has(s) && reachable.has(t)
  })
}
