...
export async function milvusFactory(node: FlowNode, context: BuildContext) {
  ...
  const embedding = await (async () => {
    const portId = embeddingVariable.id
    const edge = context.json.edges.find(e => e.target === portId)
    if (!edge) return null
    ...
    const potentialValue = upstreamResult?.[sourcePortId] ?? Object.values(upstreamResult || {})[0]
    if (potentialValue && typeof potentialValue.invoke === 'function') {
      return await potentialValue.invoke({})
    }
