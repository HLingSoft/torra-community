...
export async function messageStoreFactory(node: FlowNode, context: BuildContext) {
  ...
  const memoryEdge = context.json.edges.find((e) => e.target === memoryInputVariable.id)
  ...
  const messageEdge = context.json.edges.find((e) => e.target === messageInputVariable.id)
