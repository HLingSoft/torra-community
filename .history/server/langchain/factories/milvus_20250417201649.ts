...
export async function milvusFactory(node: FlowNode, context: BuildContext) {
  ...
  console.log('MilvusFactory', resultOutputVariable.id,dataframeOutputVariable.id)

  return {
    [resultOutputVariable.id]: RunnableLambda.from(async () => merged),
    [dataframeOutputVariable.id]:RunnableLambda.from(async () => uniqueDocuments),
  }
}
