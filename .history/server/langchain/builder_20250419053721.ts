...
export async function executeDAG(
  ...
  while (i < sortedIds.length) {
    ...
    if (node.data.type === 'IfElse') {
      ...
      // ✅ Step 1: 取 active 分支的所有 downstream（必须在剪枝前）
      const entryNodeIds = outputConnections[node.id]?.[activePortNodeId]?.map(conn => conn.toNodeId) || []
      console.log('entryNodeIds BEFORE 剪枝:', entryNodeIds)
      ...
      // ✅ Step 2: 再 collect skipPort 分支的 downstream
      ...
      // ✅ Step 3: rebuild input connections before topoSort
