...
console.log('json',json)
  ...
  const sorted = getSortedCustomNodesFromChatInput(json);
  console.log('[DAG] 拓扑排序结果:', sorted);
...
function getConnectedCustomNodes(startId, nodes, edges) {
  const visited = new Set();
  const queue = [startId];
  const reachableCustomNodes = new Set();
    ...
    for (const edge of edges) {
      if (edge.source === current && !visited.has(edge.target)) {
        ...
        if (targetNode.type === 'custom') {
          reachableCustomNodes.add(edge.target);
        }
      }
    }
  }

  return Array.from(reachableCustomNodes);
}

function topologicalSortSubgraph(nodes, edges, subNodeIds) {
  const inDegree = {};
  const graph = {};
  ...
  for (const edge of edges) {
    ...
    if (subNodeIds.includes(source) && subNodeIds.includes(target)) {
      graph[source].push(target);
      inDegree[target]++;
    }
  }

  const queue = subNodeIds.filter(id => inDegree[id] === 0);
  const sorted = [];
...
// 主函数：你只用调用这个就行
function getSortedCustomNodesFromChatInput(graphData) {
  const { nodes, edges } = graphData;
  const inputNode = Object.values(nodes).find(n => n.data.type === 'ChatInput');
  if (!inputNode) return [];

  const connectedCustoms = getConnectedCustomNodes(inputNode.id, nodes, edges);
  return topologicalSortSubgraph(nodes, edges, connectedCustoms);
}
