...
//     }
//  // 判断是否所有输入都已 resolve 成功（即不为 undefined）
// const hasAllInputs = Object.values(resolvedInput).every(v => v !== undefined)

// if (!hasAllInputs) {
//   console.log(`⏭️ 跳过节点 ${node.id}，输入不完整`)
//   continue
// }
...
export async function executeDAG(
  ...
  // 初始化起点（ChatInput）节点
  const startNodeIds = Object.values(json.nodes)
    .filter(n => n.type === 'custom' && n.data?.type === 'ChatInput')
    .map(n => n.id)

  for (const id of startNodeIds) {
    json.nodes[id].data.inputValue = inputMessage
    queue.push(id)
  }

  console.log('[DAG] 执行节点:', queue)
  ...
  while (queue.length > 0) {
    ...
    // 如果所有输入都是 undefined，跳过（避免走未激活路径）
    const hasValidInput = Object.values(resolvedInput).some(v => v !== undefined)
    if (Object.keys(connMapForNode).length > 0 && !hasValidInput) {
      console.log(`⏭️ 跳过节点 ${node.data.type} (${nodeId})，输入不完整`)
      continue
    }
    ...
    // 将有实际输出的端口对应的下游节点推入队列
    for (const conn of outputConnections[nodeId] || []) {
      if (output[conn.fromPortId] !== undefined) {
  ...
  // 查找最后一个被执行的节点
  const executedIds = Array.from(executed)
  const lastNodeId = executedIds[executedIds.length - 1]
  const lastNode = json.nodes[lastNodeId]
  const lastNodeResult = results[lastNodeId]
  const lastOutput =
...
function buildOutputConnections(json: LangFlowJson) {
  const outputMap: Record<
    string,
    { fromPortId: string; toNodeId: string; toPortId: string }[]
  > = {}
