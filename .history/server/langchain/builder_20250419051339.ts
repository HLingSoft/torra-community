...
while (i < sortedIds.length) {
  ...
  if (node.data.type === 'IfElse') {
    ...
    // ✅ 拓扑排序剩余图中还未执行、也没被剪掉的节点
    const remainingCustomNodeIds = Object.keys(json.nodes).filter(
      id =>
        json.nodes[id].type === 'custom' &&
        !executedSet.has(id) &&
        !toRemove.has(id),
    )
  
    // const extraNodes = topoSort(json.nodes, buildInputConnections(json), remainingCustomNodeIds)
    // ✅ 1. 从 activePortId 获取起点
const entryNodeIds =
outputConnections[node.id]?.[activePortId]?.map(conn => conn.toNodeId) || []

// ✅ 2. 收集 false 分支的全部 downstream
...
console.log('[IfElse] 补全分支:', activePortId, extraNodes)
