...
import { normalizeOutput} from './resolveInput'
// 初始化节点工厂注册
initFactories()
...
// console.log('json',json)
  ...
  for (let i = 0; i < sortedIds.length; i++) {
    ...
    for (const [inputPortId, conn] of Object.entries(connMapForNode)) {
      ...
      //   console.warn(`⚠️ 上游节点 [${fromNodeId}] 尚未执行或没有结果，无法传给 [${node.id}] 端口 [${inputPortId}] [${node.data.type}]`)
      //   continue
      // }
      // console.log(2)

      // ✨这里是核心修复，明确调用上游 invoke()
      // console.log('fromNodeId',fromNodeId,'fromPortId',fromPortId)
      const upstreamRunnable = results[fromNodeId][fromPortId]
      // console.log("upstreamRunnable",upstreamRunnable)
      // console.log(`🔗 [${node.id}](${node.data.type}) 端口 [${inputPortId}] 上游 [${fromNodeId}](${json.nodes[fromNodeId].data.type}) 结果:`, upstreamRunnable)
      ...
      // console.log('upstreamRunnable',upstreamRunnable)
      // if(isRunnable(upstreamRunnable)){
      //   const result = await upstreamRunnable.invoke({})
      //   console.log('result',result)
      //   resolvedInput[inputPortId] = result
      // }else{
        resolvedInput[inputPortId] = upstreamRunnable
      // }
      ...
      //   : upstreamRunnable
        // console.log(4)
        // console.log('builder.ts',resolvedInput[inputPortId])
    ...
    const nodeResult: Record<string, RunnableLike> =factoryOutput
    ...
    //判断当前节点 nodeId 是不是就是终止节点。在不在终止节点列表中
    const isTerminal = terminalNodes.some(n => n.id === node.id)
    if (isTerminal) {
      console.log('终止节点:', node.id,nodeResult[node.data.outputVariable.id])
      finalMessage =await normalizeOutput(nodeResult[node.data.outputVariable.id])
  ...
  // const nodeId=terminalNodes[0].id
  ...
  // const finalMessage = extractMessage(mergedOutputs)
...
function extractFinalMessage(output: unknown): string {
  console.log('extractFinalMessage', output)
  if (!output) {
    return ''
  }
