...
export async function executeDAG(
  ...
  for (let i = 0; i < sortedIds.length; i++) {
    ...
    //   console.log(`🔑 节点 [${node.id}](${node.data.type}) 输入:`, resolvedInput)
    // }
  ...
  const outputNode = findTerminalNode(json)
  
  console.log('🟢 终止节点:', outputNode)
  let finalMessage = ''

  if (outputNode) {
    const outputMap = results[outputNode.id] || {}

    // ✅ 获取所有 output portId
    const outputPorts: string[]
    = outputNode.data?.outputVariables?.map((v: any) => v.id)
      ?? (outputNode.data?.outputVariable?.id ? [outputNode.data.outputVariable.id] : Object.keys(outputMap))
    ...
    for (const portId of outputPorts) {
      ...
      try {
        ...
        console.error(`❌ invoke(${portId}) 出错:`, err)
        outputs.push('[Error]')
      }
    }

    // ✅ 统一格式化输出
    finalMessage = extractFinalMessage(outputs.length === 1 ? outputs[0] : outputs)
  }
...
// }
function findTerminalNode(json: LangFlowJson): FlowNode | undefined {
  const preferredTypes = ['ChatOutput', 'TextOutput']

  const candidates = Object.values(json.nodes).filter(n => n.type === 'custom')

  // 优先找 ChatOutput 等输出节点
  const preferred = candidates.find(n => preferredTypes.includes(n.data?.type))
  if (preferred) return preferred

  // 其次 fallback 到没有下游的节点（原始逻辑）
  return candidates.reverse().find(n => isTerminalNode(json, n.id))
}
