...
//   }
// }
function pruneDisconnectedCustomNodes(json: LangFlowJson, entryNodeType = 'chatInput') {
  const reachableNodeIds = new Set<string>()
  const adjacencyMap: Record<string, string[]> = {}

  // 1. 构建连接图（source → target）
  for (const edge of json.edges) {
  ...
  // 2. 找到 ChatInput 节点（type 可根据你实际定义调整）
  const entryNodes = Object.entries(json.nodes)
    .filter(([_, node]) => node.type === 'chat_input') // ← 如果你的类型是别的（如 ChatInput），这里改掉
    .map(([id]) => id)

  if (entryNodes.length === 0) {
    console.warn('[pruneDisconnected] No chat_input node found')
    return
  }
  ...
  while (queue.length > 0) {
    ...
    const targets = adjacencyMap[current] || []
