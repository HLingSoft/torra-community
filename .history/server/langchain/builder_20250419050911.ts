...
while (i < sortedIds.length) {
  ...
  if (node.data.type === 'IfElse') {
    ...
    // ✅ 2. 清理 sortedIds 中不该执行的节点
    sortedIds = sortedIds.filter(id => !toRemove.has(id))
  
    // ✅ 3. 找出激活分支的“首节点们”
    const newBranchStarts: string[] =
      outputConnections[node.id]?.[activePortId]?.map(conn => conn.toNodeId) || []
  
    // ✅ 4. 找到当前已执行的节点，避免重复排入
    const executedSet = new Set(sortedIds.slice(0, i + 1))
  
    // ✅ 5. 对新分支节点拓扑排序（只针对还没排过的）
    const extraNodes = topoSort(
      json.nodes,
      buildInputConnections(json),
      newBranchStarts.filter(id => !executedSet.has(id)),
    )
  
    // ✅ 6. 插入 false 分支的路径回 sortedIds，紧跟在当前 IfElse 节点后面
    sortedIds = [...sortedIds.slice(0, i + 1), ...extraNodes]
  
    console.log('[IfElse] 剪枝:', skipPortId, '分支', toRemove)
    console.log('[IfElse] 插入分支:', activePortId, '分支', extraNodes)
  }
