...
console.log('json',json)
  ...
  const startCustomIds = getReachableCustomNodeIds(json)
  const sortedIds = topoSort(json.nodes, inputConnections, startCustomIds)
...
function getReachableCustomNodeIds(json: LangFlowJson): string[] {
  ...
  // 从起点出发，广度优先搜索所有 reachable 的 custom 节点
  while (queue.length > 0) {
    const currentId = queue.shift()!
    for (const edge of json.edges) {
      if (edge.source === currentId) {
        const targetCustomId = edge.data.targetParent
        if (targetCustomId && !visited.has(targetCustomId)) {
          visited.add(targetCustomId)
          queue.push(targetCustomId)
        }
      }
    }
  }
...
function pruneDisconnectedCustomNodes(json: LangFlowJson, entryType = 'ChatInput') {
  ...
  for (const node of Object.values(json.nodes)) {
    if (node.type === 'custom') {
      // 找出所有属于该 custom 节点的子节点（端口）
      const childPorts = Object.values(json.nodes).filter(
        (n) => n.data?.parentNode === node.id
      )

      // 判断这些子节点是否有参与任何 edge 的连接
      const connected = json.edges.some((edge) =>
        childPorts.some(
          (port) => edge.source === port.id || edge.target === port.id
        )
      )

      // 如果完全没连，就把 custom 和它的子节点都删掉
      if (!connected) {
        nodesToDelete.add(node.id)
        for (const port of childPorts) {
          nodesToDelete.add(port.id)
        }
      }
    }
  }
  ...
  // 可选：删除 orphan 的边（万一有无主边）
  json.edges = json.edges.filter(
    (edge) => json.nodes[edge.source] && json.nodes[edge.target]
  )

  console.log(`[prune] ✅ 清理 ${nodesToDelete.size} 个节点（custom + 子端口）`)
}
