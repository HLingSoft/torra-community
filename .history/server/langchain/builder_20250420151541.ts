...
if (node.data.type === 'IfElse') {
  ...
  // ✅ 原始 activeSet（从 false 分支开始的所有后代）
  ...
  for (const conn of outputConnections[node.id]?.[activePortNodeId] || []) {
    ...
    downstream.forEach(id => rawActiveSet.add(id))
  }

  // ✅ 收集从 activeSet 出发，向上的依赖祖先，形成完整依赖集
  const fullActiveSet = collectUpstream([...rawActiveSet], inputConnections)
  fullActiveSet.forEach(id => rawActiveSet.add(id))

  const activeSet = rawActiveSet

  // ✅ 剪除 skip 分支中不在 activeSet 的独立节点
  const toRemove = new Set<string>()
  for (const conn of outputConnections[node.id]?.[skipPortNodeId] || []) {
    const downstream = collectExclusiveDownstream(conn.toNodeId, outputConnections, activeSet)
    downstream.forEach(id => toRemove.add(id))
  }

  const prunedPending = pending.filter(id => !toRemove.has(id))
  const toAdd = [...activeSet].filter(id => !executed.has(id) && !toRemove.has(id))
