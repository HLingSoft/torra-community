    ...
    if (variable.connected) {
      ...
      if(potentialValue){
        if(variable.allowedTypes?.includes('Message') || variable.allowedTypes?.includes('Prompt Message')){
          resolved = await normalizeToString(potentialValue)
        }else{
...
export async function normalizeToString(value: any): Promise<any> {
  if (value?.lc === 1 && value?.type === 'constructor') {
    try {
      const instance = await load(value)
      if (instance?.content) return instance.content
    } catch (e) {
      console.warn('Failed to load LangChain object:', e)
    }
  }
