...
export async function executeDAG(
  ...
  while (pending.length > 0) {
    ...
    if (node.data.type === 'IfElse') {
      ...
      // const toAdd = [...activeSet].filter(id => !executed.has(id))
      const upstream = collectUpstream([...activeSet], inputConnections)
const allToAdd = [...new Set([...activeSet, ...upstream])].filter(id => !executed.has(id))

      inputConnections = buildInputConnections(json)
      const extra = topoSortSafe(json.nodes, inputConnections, allToAdd)
