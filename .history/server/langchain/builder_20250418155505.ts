...
function isStartNode(json: LangFlowJson, nodeId: string): boolean {
  const node = json.nodes[nodeId]
  return ['ChatInput', 'TextInput', 'QuestionInput'].includes(node?.data?.type)
}
...
//   }
// }
function pruneDisconnectedCustomNodes(json: LangFlowJson, entryType = 'ChatInput') {
  const reachableCustoms = new Set<string>()

  // Step 1: 构建 portId → nodeId 映射
  const portToNodeMap = new Map<string, string>()
  for (const [nodeId, node] of Object.entries(json.nodes)) {
    if (!node.data) continue

    const ports = Object.values(node.data)
      .filter((v: any) => v && typeof v === 'object' && typeof v.id === 'string') as { id: string }[]

    for (const port of ports) {
      portToNodeMap.set(port.id, nodeId)
    }
  }

  // Step 2: 构建节点之间的连通图
  const nodeGraph: Record<string, Set<string>> = {}
  for (const edge of json.edges) {
    const fromNode = portToNodeMap.get(edge.source)
    const toNode = portToNodeMap.get(edge.target)
    if (fromNode && toNode) {
      if (!nodeGraph[fromNode]) nodeGraph[fromNode] = new Set()
      nodeGraph[fromNode].add(toNode)
    }
  }

  // Step 3: 从所有 input 类型节点出发，BFS 遍历
  const entryNodes = Object.entries(json.nodes)
    .filter(([_, node]) => node.type === 'input')
    .map(([id]) => id)
  ...
  while (queue.length) {
    ...
    if (json.nodes[current]?.type === 'custom') {
      reachableCustoms.add(current)
    }

    const next = nodeGraph[current] || []
  ...
  // Step 4: 删除所有不是 custom 或 不在 reachableCustoms 中的节点
  for (const nodeId of Object.keys(json.nodes)) {
    const node = json.nodes[nodeId]
    if (node.type !== 'custom' || !reachableCustoms.has(nodeId)) {
      delete json.nodes[nodeId]
    }
  }

  // Step 5: 清理边，只保留连接的两个 node 都是 reachableCustom 的
  json.edges = json.edges.filter(edge => {
    const s = portToNodeMap.get(edge.source)
    const t = portToNodeMap.get(edge.target)
    return s && t && reachableCustoms.has(s) && reachableCustoms.has(t)
  })

  console.log(`[prune] ✅ 最终保留 ${reachableCustoms.size} 个 custom 节点`)
}
