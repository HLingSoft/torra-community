...
export async function resolveInputVariables(
  ...
  for (const variable of variables) {
    ...
    if (variable.connected) {
      ...
      if(potentialValue){
        // console.log('variable.forceStringify', variable.forceStringify )
        if(variable.forceStringify && (variable.allowedTypes?.includes('Message') || variable.allowedTypes?.includes('Prompt Message'))){
          // console.log('执行了', variable.name )
...
export  function normalizeToString(value: any): any {
