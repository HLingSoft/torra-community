...
export async function resolveInputVariables(
  ...
  for (const variable of variables) {
    ...
    if (variable.connected) {
      ...
      if(variable.id ==='s58sqfahk2'){
        console.log('context',context.resolvedInput[variable.id])
      }
      // console.log('potentialValue',potentialValue)
      if(potentialValue){
        // console.log(`🔑 变量 [${variable.name}] [${variable.id}] 通过 resolvedInput 解析:`, potentialValue)
        
        resolved = extractMessage(potentialValue)
        // console.log(`🔑 变量 [${variable.name}] 通过 resolvedInput 解析:`, resolved)
      ...
      // console.log('执行 resolveInputVariables')
...
function extractMessage(output: unknown): any {
  if (!output) {
    return ''
  }
