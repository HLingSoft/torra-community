...
// ✅ IfElse 分支控制逻辑（最终修正：从端口出发明确分支子图）
// ✅ IfElse 分支控制逻辑（最终修正：确保补全节点严格限定于 active 分支及依赖）
if (node.data.type === 'IfElse') {
  ...
  // ✅ activeSet = 从 false 分支起点出发的子图（子图 + 所有依赖）
  const rawActiveSet = new Set<string>()
  for (const id of activeEntryIds) {
    collectDownstream(id, outputConnections).forEach(i => rawActiveSet.add(i))
  }

  const upstream = collectUpstream([...rawActiveSet], inputConnections)
  upstream.forEach(id => rawActiveSet.add(id))
  const activeSet = rawActiveSet

  // ✅ skipSet = 从 true 分支起点出发的完整子图
  const skipSet = new Set<string>()
  for (const id of skipEntryIds) {
    collectDownstream(id, outputConnections).forEach(i => skipSet.add(i))
  }

  // ✅ toRemove = true 分支中那些不属于 active 分支的节点
  ...
  console.log('prunedPending:', prunedPending)
