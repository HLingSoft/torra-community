...
// 计算 zoom，设定一个范围，比如 min 0.2 ~ max 1.2，根据 nodes 数量动态调整
const dynamicZoom = computed(() => {
  const baseZoom = 1
  const maxNodes = 10 // 假设 100 个节点是“非常多”
  const count = nodes.value.length

  // 根据节点数量，缩放在 minZoom 到 baseZoom 之间线性变化
...
background-size: 50px 50px; background-color: #1e1e1e;"
      >
        <VueFlow :nodes="nodes" :edges="edges" :default-viewport="{ zoom:dynamicZoom}" :zoom="dynamicZoom" :max-zoom="4" :min-zoom="0.1">
