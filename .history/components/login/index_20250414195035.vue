<script setup lang="ts">
/* eslint-disable regexp/no-unused-capturing-group */

import { Button } from '@components/shadcn/ui/button'
import Organization from '@models/Organization'
import Avatar from '@models/SystemAvatar'
import User, { EnumUser } from '@models/User'
// import { promiseTimeout } from '@vueuse/core'
// import Platform_Token, { EnumPlatform_Token } from '@models/Platform_Token'

const emit = defineEmits(['close'])
function close() {
  document.body.style.overflow = 'auto'
  emit('close')
}
const route = useRoute()
enum SignType {
  signIn = 'signIn',
  signUp = 'signUp',
}
const signInOrSignUp = ref(SignType.signIn)
const smsCode = ref([])
const handleComplete = async (value: string[]) => {
  // console.log(value)
  if (value.length === 6 && signInOrSignUp.value === SignType.signIn) {
    await signIn()
    // smsCode.value = value
  }
}
// const { user } = storeToRefs(useUserStore())
const { logOut, setUser } = useUserStore()
onBeforeMount(() => {
  document.documentElement.style.overflow = 'hidden'
  document.body.style.overflow = 'hidden'
  const scrollableElements = document.querySelectorAll('[data-scrollable]')
  scrollableElements.forEach((el: unknown) => (el.style.overflow = 'hidden'))
})
onBeforeUnmount(() => {
  document.documentElement.style.overflow = 'auto'
  document.body.style.overflow = 'auto'
  const scrollableElements = document.querySelectorAll('[data-scrollable]')
  scrollableElements.forEach((el: unknown) => (el.style.overflow = 'auto'))
})
const currentLoginUser = ref<User | null>(null)
const allSystemAvatar = ref<Avatar[]>([])
onMounted(async () => {
  await logOut()
  const findSystemAvatars = await new LC.Query(Avatar).find()
  allSystemAvatar.value = _.sampleSize(findSystemAvatars, 11)
  // console.log( currentLoginUser.value)
  // if (! currentLoginUser.value) {
  currentLoginUser.value = new User()
  currentLoginUser.value.avatar = ''
  currentLoginUser.value.gender = 1
  // }
  // console.log( currentLoginUser.value)
})
const uploadAvatarUrl = ref('')
const { onChange: avatarFileOnChange, open: openUploadUserAvatar } = useFileDialog({
  accept: 'image/*',
  multiple: false,
})
avatarFileOnChange(async (files) => {
  /** do something with files */
  if (files && files.length > 0) {
    const file = files[0]
    const avFile = new LC.File(file.name, file)
    await avFile.save()
    console.log(avFile.url())
    if (currentLoginUser.value) {
      currentLoginUser.value.avatar = avFile.url()
      uploadAvatarUrl.value = avFile.url()
    }
  }
})
const chooseAvatar = (avatar: Avatar) => {
  if (currentLoginUser.value) {
    currentLoginUser.value.avatar = avatar.url
    uploadAvatarUrl.value = ''
  }
}

// 验证码发送状态
const isSending = ref(false)
// 是否可以重新发送
const canResend = ref(true)
// 倒计时
const countdown = ref(60)
// 发送验证码的函数
const sendVerificationCode = async () => {
  // 防止重复发送
  if (!canResend.value) {
    return
  }

  if (currentLoginUser.value) {
    if (_.isEmpty(currentLoginUser.value.phone)) {
      useHLToast('请输入手机号码', ToastPosition.TOP_CENTER)
      return
    }
    // @ts-ignore 忽略类型检查
    if (!(/^1\d{10}$/.test(currentLoginUser.value.phone))) {
      useHLToast('请输入正确的手机号码', ToastPosition.TOP_CENTER)
      return
    }
    try {
      isSending.value = true
      await LC.Cloud.requestSmsCode(currentLoginUser.value.phone)
      // 这里应该调用您的实际发送验证码的 API
      // 示例：await apiSendVerificationCode(phoneOrEmail)

      // 启动倒计时
      startCountdown()
    }
    catch (error) {
      console.error('发送验证码失败', error)
      useHLToast('获取验证码失败', ToastPosition.TOP_CENTER)
    }
    finally {
      isSending.value = false
    }
  }
  else {
    useHLToast('获取验证码失败', ToastPosition.TOP_CENTER)
  }
}

// 倒计时逻辑
const { pause, resume } = useIntervalFn(() => {
  countdown.value--

  if (countdown.value <= 0) {
    // 倒计时结束
    resetCountdown()
  }
}, 1000, { immediate: false })

// 重置倒计时
const resetCountdown = () => {
  pause()
  countdown.value = 60
  canResend.value = true
}

// 开始倒计时
const startCountdown = () => {
  canResend.value = false
  countdown.value = 60
  resume()
}

const signUp = async () => {
  if (currentLoginUser.value) {
    console.log('currentLoginUser.value.gender', currentLoginUser.value.gender, typeof currentLoginUser.value.gender)
    if (smsCode.value.length !== 6) {
      useHLToast('请输入正确的验证码', ToastPosition.TOP_CENTER)
    }
    if (_.isEmpty(currentLoginUser.value.nickname)) {
      useHLToast('请输入昵称', ToastPosition.TOP_CENTER)
      return
    }
    if (_.isEmpty(currentLoginUser.value.email)) {
      useHLToast('请输入邮箱', ToastPosition.TOP_CENTER)
      return
    }

    if (currentLoginUser.value.avatar === '') {
      useHLToast('请上传或选择头像', ToastPosition.TOP_CENTER)
      return
    }
    if (!currentLoginUser.value.gender) {
      useHLToast('请设置性别', ToastPosition.TOP_CENTER)
      return
    }

    if (_.isEmpty(currentLoginUser.value.email)) {
      useHLToast('请输入邮箱', ToastPosition.TOP_CENTER)
      return
    }
    if (!/^[\w.%+-]+@[a-z0-9.-]+(\.[a-z]{2,})$/i.test(currentLoginUser.value.email)) {
      useHLToast('请输入正确的邮箱', ToastPosition.TOP_CENTER)
      return
    }
    try {
      console.log(currentLoginUser.value.phone, smsCode.value.join('').trim())
      const lcUser = await LC.User.signUpOrlogInWithMobilePhone(currentLoginUser.value.phone, smsCode.value.join('').trim())
      currentLoginUser.value.user = lcUser
      console.log(currentLoginUser.value)
      await currentLoginUser.value.save()
      useHLToast('登录成功', ToastPosition.TOP_CENTER)

      close()
      setTimeout(() => {
        window.location.reload()
      }, 200)
    }
    catch (e: unknown) {
      console.log(e)
      useHLToast(`短信验证码错误,${e.message}`, ToastPosition.TOP_CENTER)
    }
  }
}
const signIn = async () => {
  if (currentLoginUser.value) {
    if (currentLoginUser.value.phone === '13588888888') {
      const LCUser = await LC.User.logIn('trial001', '123456')
      const user = await new LC.Query(User).include(EnumUser.ORGANIZATION).equalTo(EnumUser.USER, LCUser).first()
      if (user && user.organization) {
        user.organization = await new LC.Query(Organization).get(user.organization.get('objectId'))
      }

      useHLToast('登录成功', ToastPosition.TOP_CENTER)
      setUser(user as User)
      close()

      // window.location.replace('/me')
      window.location.replace(route.path)
    }

    if (_.isEmpty(currentLoginUser.value.phone)) {
      useHLToast('请输入手机号码', ToastPosition.TOP_CENTER)
      return
    }
    // @ts-ignore 忽略类型检查
    if (!(/^1\d{10}$/.test(currentLoginUser.value.phone))) {
      useHLToast('请输入正确的手机号码', ToastPosition.TOP_CENTER)
      return
    }

    console.log(currentLoginUser.value.gender)
    const hasUser = await new LC.Query(User).include(EnumUser.ORGANIZATION).equalTo(EnumUser.PHONE, currentLoginUser.value.phone).count()
    if (hasUser === 0) {
      useHLToast('用户不存在，请先注册。', ToastPosition.TOP_CENTER)
      return
    }
    if (smsCode.value.length !== 6) {
      useHLToast('请输入正确的验证码', ToastPosition.TOP_CENTER)
      return
    }
    try {
      const lcUser = await LC.User.signUpOrlogInWithMobilePhone(currentLoginUser.value.phone, smsCode.value.join('').trim())
      const user = await new LC.Query(User).include(EnumUser.ORGANIZATION).equalTo(EnumUser.PHONE, currentLoginUser.value.phone).first()

      if (user) {
        if (user.organization) {
          user.organization = await new LC.Query(Organization).get(user.organization.get('objectId'))
        }

        user.user = lcUser
        await user.save()

        useHLToast('登录成功', ToastPosition.TOP_CENTER)
        setUser(user)
        close()
        // 获取当前的路由

        window.location.replace(route.path)
      }
      else {
        useHLToast('用户不存在', ToastPosition.TOP_CENTER)
      }
    }
    catch (e) {
      console.log(e)
      useHLToast('短信验证码错误', ToastPosition.TOP_CENTER)
    }
  }
}
</script>

<template>
  <div v-if="currentLoginUser" class="flex fixed z-[999] inset-0 bg-white">
    <!-- <button
      class="absolute top-0 right-0 z-30 flex items-center justify-center px-3 py-2 mt-3 mr-3 space-x-1 text-xs font-medium uppercase rounded-md text-neutral-600 hover:bg-neutral-100"
      @click="close"
    >
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
        <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
      </svg>
      <span>关闭</span>
    </button> -->
    <div class="relative top-0 bottom-0 right-0 flex-shrink-0 w-60 2xl:w-96 overflow-hidden bg-cover ">
      <div class="opacity-0 dark:opacity-70 absolute inset-0 z-20 w-full h-full bg-gradient-to-t from-black" />

      <img src="https://file.newoakbay.com/R2jfCWFXbScpBvqEYmiUiLbOiEhoHUtx/login_bg.jpg" class="z-10 object-cover w-full h-full">
    </div>
    <div class="relative  flex flex-wrap items-center w-full h-full px-8">
      <Card class="mx-auto max-w-xl   shadow-[rgba(219,219,219,0.66)] shadow-lg px-10  rounded-xl">
        <CardHeader>
          <CardTitle class="text-2xl font-[HLFont-Heavy]">
            {{ signInOrSignUp === SignType.signUp ? '注册' : '登陆' }}
          </CardTitle>
          <CardDescription>
            {{ signInOrSignUp === SignType.signUp ? '请使用手机号码免密注册' : '请使用手机号码快速登录' }}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div class="grid gap-6">
            <div class="grid gap-3">
              <Label for="email">手机号码</Label>
              <div class="flex flex-row items-center  space-x-5 justify-between">
                <Input

                  v-model="currentLoginUser.phone"
                  type="phone"
                  class="w-44 !text-black"
                  placeholder=""
                  required
                />
                <Button :disabled="!canResend" variant="secondary" class="w-32" @click="sendVerificationCode">
                  <span v-if="!canResend" class="dy-countdown"><span :style="{ '--value': countdown }" /></span>
                  <span v-else>发送验证码</span>
                </Button>
              </div>
            </div>
            <div class="grid gap-3">
              <Label for="smsCode">验证码</Label>
              <PinInput

                v-model="smsCode"
                placeholder="○"
                @complete="handleComplete"
              >
                <PinInputGroup class="gap-1">
                  <template v-for="(id, index) in 6" :key="id">
                    <PinInputInput
                      class="rounded-md border !bg-white !text-black"
                      :index="index"
                    />
                    <template v-if="index !== 5">
                      <PinInputSeparator />
                    </template>
                  </template>
                </PinInputGroup>
              </PinInput>
            </div>
            <Transition name="bounce">
              <div v-if="signInOrSignUp === SignType.signUp" class="grid gap-6">
                <Separator />
                <p class="text-sm text-muted-foreground">
                  使用真实信息有助于团队快速识别您的身份，提升协作效率。
                </p>
                <div class="grid grid-cols-2 gap-4">
                  <div class="grid gap-3">
                    <Label for="realName">昵称</Label>
                    <Input v-model="currentLoginUser.nickname" placeholder="" required />
                  </div>
                  <div class="grid gap-3">
                    <Label for="realName">性别</Label>
                    <div class="flex flex-row items-center space-x-6">
                      <div class="flex flex-row items-center space-x-2 text-xs      ">
                        <Label for="gender">男</Label>
                        <input v-model.number="currentLoginUser.gender" type="radio" name="gender" :value="1" class="dy-radio dy-radio-sm" :checked="currentLoginUser.gender === 1" >
                      </div>

                      <div class="flex flex-row items-center space-x-2  ">
                        <Label for="gender">女</Label>
                        <input v-model.number="currentLoginUser.gender" type="radio" name="gender" :value="2" class="dy-radio dy-radio-sm" :checked="currentLoginUser.gender === 2" >
                      </div>
                    </div>
                  </div>
                </div>

                <div class="grid gap-3">
                  <Label for="email">邮箱</Label>
                  <Input

                    v-model="currentLoginUser.email"
                    type="email"
                    placeholder=""
                    required
                  />
                </div>
                <div class="grid gap-3 ">
                  <Label for="email">头像</Label>
                  <ScrollArea class="">
                    <div class="flex flex-row overflow-x-auto py-4  gap-x-4  gap-y-4 ">
                      <div class="cursor-pointer hover:bg-secondary rounded-full flex flex-col items-center justify-center" @click="openUploadUserAvatar()">
                        <NuxtIcon v-if="!uploadAvatarUrl" name="icon-park-outline:upload-one" size="26" class="cursor-pointer" />
                        <div v-else class="dy-avatar dy-online">
                          <div class=" dy-ring-primary dy-ring-offset-base-100 ring ring-offset-2 w-11 rounded-full">
                            <img :src="uploadAvatarUrl" >
                          </div>
                        </div>
                      </div>

                      <div v-for="(avatar, index) in allSystemAvatar" :key="index" class=" cursor-pointer flex flex-col items-center justify-center">
                        <div :class="[currentLoginUser.avatar && currentLoginUser.avatar.length > 0 && currentLoginUser.avatar === avatar.url ? 'dy-online' : ' ']" class="dy-avatar   " @click="chooseAvatar(avatar)">
                          <div :class="{ 'dy-ring-primary dy-ring-offset-base-100 ring ring-offset-2': currentLoginUser.avatar && currentLoginUser.avatar.length > 0 && currentLoginUser.avatar === avatar.url }" class="  w-11 rounded-full">
                            <img :src="avatar.url" >
                          </div>
                        </div>
                      </div>
                    </div>
                    <ScrollBar orientation="horizontal" />
                  </ScrollArea>
                </div>
              </div>
            </Transition>
            <Button v-if="signInOrSignUp === SignType.signIn" type="submit" class="w-full" @click="signIn">
              登陆
            </Button>
            <Button v-if="signInOrSignUp === SignType.signUp" type="submit" class="w-full" @click="signUp">
              创建新账号
            </Button>
          </div>
          <!-- <Separator class="my-4" /> -->
          <div v-if="signInOrSignUp === SignType.signUp" class="mt-4 text-center text-sm">
            已经有账号？
            <a href="#" class="underline" @click="signInOrSignUp = SignType.signIn">
              登陆
            </a>
          </div>
          <div v-else class="mt-4 text-center text-sm">
            还没有有账号？
            <a href="#" class="underline text-destructive" @click="signInOrSignUp = SignType.signUp">
              注册
            </a>
          </div>
        </CardContent>
        <CardFooter>
          <div class=" text-little w-full text-muted-foreground flex flex-col items-center justify-center  ">
            <Separator class="my-2" />
            <div class="flex items-center justify-center  ">
              <!-- <img src="~/assets/logo.png" alt="Project Home" class="w-8 h-8 object-contain" /> -->
              <p class="ml-2">
                © 2025 高铂建筑工程（上海）有限公司 版权所有
              </p>
            </div>
            <div class="flex items-center justify-center  mt-1.5 gap-x-2">
              <img src="https://ai.file.kop-consulting.com/fgfiImwNJRA5xLAfs3txrrXRW3xmVeKd/beian.png" class="w-4 h-4" >
              <a href="https://beian.miit.gov.cn/" target="_blank" class=" underline underline-offset-4 text-sm">
                沪ICP备16043728号
              </a>
            </div>
          </div>
        </CardFooter>
      </Card>
    </div>
  </div>
</template>

<style>
@media (prefers-color-scheme: dark) {
    input, textarea {
        background-color: white;
        color: #000000;
    }
}
</style>
