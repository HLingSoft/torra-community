...
// let loginCheckInterval: ReturnType<typeof setInterval>
...
onMounted(async () => {
  if (!isMobile.value) {
    await requestQRCode()
    allAvatar.value = await new LC.Query(Avatar).find()
  }
})
...
//   // useHLToast('请使用手机号码登录')
//   requestLoginSmsCodeCountdown.value = 0
//   const platformToken = await new LC.Query(Platform_Token).equalTo(EnumPlatform_Token.APPID, 'wxe1e986bc4c8485eb').first()
//   if (platformToken && openid.value) {
//     const authData = {
//       openid: openid.value,
//       access_token: platformToken.token,
//     }
//     console.log(authData)
//     const randomAvatar = _.sample(allAvatar.value) as Avatar
//     const lcUser = await LC.User.loginWithAuthData(authData, 'mp')
//     const user = new User()
//     user.user = lcUser
//     user.mp_openId = openid.value
//     user.validated = true
//     user.isSubscribe = true
//     user.name = randomAvatar.name
//     user.invalid = false
//     user.phone = phoneNumber.value
//     user.avatar = randomAvatar.url
//     await user.save()
//     await user.fetch()
...
async function doLogin() {
  try {
    ...
    user.invalid = false
    user.phone = phoneNumber.value
    user.avatar = (_.sample(allAvatar.value) as Avatar).url
    await user.save()
    await user.fetch()
    useHLToast('登录成功', ToastPosition.TOP_CENTER)
    useUserStore().setUser(user)
...
<template>
  <div v-if="!isMobile" class="flex fixed z-[999] inset-0 bg-white">
    <button
  ...
  <div v-else class="flex fixed z-[999999] h-20 w-screen bg-secondary  items-center justify-center bottom-0 rounded-t-2xl">
    <div>请在电脑端使用</div>
  </div>
</template>
