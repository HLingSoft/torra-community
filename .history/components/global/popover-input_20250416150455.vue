        ...
        <Command>
          ...
          <CommandList>
            ...
            <CommandGroup heading="Suggestions">
              <CommandItem class="!py-2" v-for="variable in variables" :key="variable.name" :value="variable.value" @select="selectVariable(variable)">
                <div class="flex flex-col   ">
                  <span class="font-medium text-sm">{{ variable.name }}</span>
                  <span class="text-xs text-muted-foreground    line-clamp-1  ">{{ variable.value }}</span>
                </div>
              </CommandItem>
            </CommandGroup>
