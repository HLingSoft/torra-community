  ...
  <Card v-if="currentNode && currentNode.data" class="!pb-0 w-96 text-white bg-[#18181B]  rounded-lg  flex flex-col focus:outline-none  focus:shadow-lg focus:shadow-[#000000]   focus:border focus:border-[#27272A]">
    ...
    <CardContent class="text-white space-y-8 -mt-8  flex-1 nodrag nopan cursor-auto ">
      ...
      <div ref="systemMessageRef">
        ...
        <div class="w-full  mt-5">
          <EditTextDialog class="w-full" :disabled="currentNode.data.systemMessageVariable.connected" :model-value="currentNode.data.systemMessageVariable.value || ''" placeholder="Typing something" @save="(val) => currentNode!.data!.systemMessageVariable.value = val" />
        </div>
      </div>
        ...
        <div class="w-full flex items-center justify-between  mt-4">
          <Select v-model="currentNode.data.modelName" class="w-full">
            ...
            <SelectContent class="w-full dark">
              <SelectGroup>
                <SelectItem v-for="model in models" :key="llm.id" :value="llm.id">
                  {{ model.name }}
                </SelectItem>
              </SelectGroup>
            </SelectContent>
    ...
    <div ref="languageModelRef" class="bg-[#27272A]   -mt-2 rounded-b-lg py-2 pl-5 pr-10  flex items-center justify-center">
      <div class="w-full h-full   flex items-center  justify-between">
        ...
        <NuxtIcon v-else name="lets-icons:view-hide-duotone" size="24" class="cursor-pointer" @click="currentNode.data.languageModelOutputVariable.show = true" />
