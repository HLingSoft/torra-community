...
watch(() => currentNode.value?.data?.methodType, async (val, oldVal) => {
  ...
  if (val === 'get') {
    ...
    // ✅ 删除 body input port
    const bodyId = currentNode.value.data!.bodyVariable.id
    removePort(bodyId)

    // ✅ 删除 dataOutputVariable output port
    const dataId = currentNode.value.data!.dataOutputVariable.id
    removePort(dataId)

    // ✅ 更新 output port 位置（使用 footer）
    await nextTick()
    if (footer.value) {
