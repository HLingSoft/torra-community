...
const bodyVariableClonedData= ref<{ key: string, value: any }[]>([])
const saveBodyVariable = () => {
  if (!currentNode.value) {
    return
  }
//   currentNode.value!.data!.bodyVariable.value = currentEditVariable!.value
  ...
  <Dialog v-model:open="bodyVariableIsOpen">
      <DialogContent class="dark  flex flex-col text-white w-full !max-w-7xl h-[80vh]">
        ...
        <div class="flex-1 overflow-auto">
              <KeyValueEditor class="h-full"  v-model="currentNode?.data?.bodyVariable.value"/>
        </div>
