...
watch(() => currentNode.value?.data?.methodType, async (val, oldVal) => {
  ...
  if (val === 'get') {
    ...
    // ✅ 移除 dataOutputVariable port
    removePort(currentNode.value.data.dataOutputVariable.id)

    // ✅ 可选：DataFrame 也一起移除（如有需要）
    removePort(currentNode.value.data.dataFrameOutputVariable.id)

    // ✅ 更新主 output port 位置
    await nextTick()
    if (dataOutputVariableRef.value && currentNode.value.data.outputVariable.id) {
    ...
    if (dataOutputVariableRef.value) {
      const id = nanoLowercaseAlphanumericId(10)
      currentNode.value.data.dataOutputVariable.id = id
      addOutputPort(nodeId, id, 'pink', dataOutputVariableRef.value.offsetTop + dataOutputVariableRef.value.clientHeight / 2)
    }

    if (dataFrameOutputVariableRef.value) {
      const id = nanoLowercaseAlphanumericId(10)
      currentNode.value.data.dataFrameOutputVariable.id = id
      addOutputPort(nodeId, id, 'pink', dataFrameOutputVariableRef.value.offsetTop + dataFrameOutputVariableRef.value.clientHeight / 2)
    }
  }
}, { immediate: true })
