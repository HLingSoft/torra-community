...
const urlInputVariable = ref<HTMLElement | null>(null)
...
onMounted(async () => {
  ...
  if (urlInputVariable.value && !node.data.saved) {
    currentNode.value.data!.urlInputVariable.id = nanoLowercaseAlphanumericId(10)
    addInputPort(props.id!, currentNode.value!.data!.urlInputVariable.id, 'aquamarine', urlInputVariable.value.offsetTop + urlInputVariable.value.clientHeight / 2)
  }
...
<template>
  <Card v-if="currentNode && currentNode.data" class="!pb-0 w-96 text-white bg-[#18181B]  rounded-lg  flex flex-col focus:outline-none  focus:shadow-lg focus:shadow-[#000000]   focus:border focus:border-[#27272A]">
    ...
    <CardContent class="text-white space-y-8 -mt-8  flex-1 nodrag nopan cursor-auto ">
      ...
      <div ref="urlInputVariable">
        ...
        <div class="w-full  mt-5">
          <PopoverInput
          class="w-full"
          :disabled="currentNode.data.urlInputVariable.connected"
          :model-value="currentNode.data.urlInputVariable.value || ''"
          
          placeholder="Typing something"
          @save="(val) => currentNode!.data!.urlInputVariable.value = val" >
          </PopoverInput>
        </div>
      </div>
    ...
    <div ref="dataFrameOutputVariableRef" class="bg-[#27272A]   py-2 pl-5 pr-10  flex items-center justify-center">
      <div class="w-full h-full   flex items-center  justify-between">
