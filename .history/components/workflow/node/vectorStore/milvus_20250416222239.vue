...
onMounted(async () => {
  ...
  if(partitionKeyRef.value && !data.saved) {
    data.partitionKeyVariable.id = nanoLowercaseAlphanumericId(10)
    addInputPort(id, data.partitionKeyVariable.id, 'aquamarine', partitionKeyRef.value.offsetTop + partitionKeyRef.value.clientHeight / 2)
  }
...
<template>
  <Card v-if="currentNode && currentNode.data" class="!pb-0 w-96 text-white bg-[#18181B]  rounded-lg  flex flex-col focus:outline-none  focus:shadow-lg focus:shadow-[#000000]   focus:border focus:border-[#27272A]">
    ...
    <CardContent class="text-white space-y-8 -mt-8  flex-1 nodrag nopan cursor-auto ">
      ...
      <div ref="partitionKeyRef">
        <div class="flex flex-row items-center space-x-2">
          <p>Partition Key</p>
          <NuxtIcon name="clarity:info-line" size="20" />
        </div>
        <div class="w-full  mt-5">
          <EditTextDialog class="w-full" :disabled="currentNode.data.partitionKeyVariable.connected" :model-value="currentNode.data.partitionKeyVariable.value || ''" placeholder="Typing something" @save="(val) => currentNode!.data!.partitionKeyVariable.value = val" />
        </div>
      </div>
