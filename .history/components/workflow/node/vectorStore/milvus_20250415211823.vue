...
onMounted(async () => {
  ...
  if(embeddingRef.value && !data.saved) {
    ...
    addInputPort(id, data.embeddingVariable.id, 'aquamarine', embeddingRef.value.offsetTop + embeddingRef.value.clientHeight / 2)
  }
  if(ingestDataRef.value && !data.saved) {
    data.ingestDataVariable.id = nanoLowercaseAlphanumericId(10)
    addInputPort(id, data.ingestDataVariable.id, 'aquamarine', ingestDataRef.value.offsetTop + ingestDataRef.value.clientHeight / 2)
  }
  ...
  <Card v-if="currentNode && currentNode.data" class="!pb-0 w-96 text-white bg-[#18181B]  rounded-lg  flex flex-col focus:outline-none  focus:shadow-lg focus:shadow-[#000000]   focus:border focus:border-[#27272A]">
    ...
    <div ref="resultOutputRef" class="bg-[#27272A]   py-2 pl-5 pr-10  flex items-center justify-center">
      <div class="w-full h-full   flex items-center  justify-between">
        <NuxtIcon v-if="currentNode.data.resultOutputRef.show" name="lets-icons:view-duotone" size="24" class="cursor-pointer" @click="currentNode.data.resultOutputRef.show = false" />
