...
onMounted(async () => {
  ...
  if(embeddingRef.value && !data.saved) {
    ...
    addInputPort(id, data.embeddingVariable.id, 'yellow', embeddingRef.value.offsetTop + embeddingRef.value.clientHeight / 2-20)
  }
  if(ingestDataRef.value && !data.saved) {
    data.ingestDataVariable.id = nanoLowercaseAlphanumericId(10)
    addInputPort(id, data.ingestDataVariable.id, 'yellow', ingestDataRef.value.offsetTop + ingestDataRef.value.clientHeight / 2-20)
  }
  ...
  <Card v-if="currentNode && currentNode.data" class="!pb-0 w-96 text-white bg-[#18181B]  rounded-lg  flex flex-col focus:outline-none  focus:shadow-lg focus:shadow-[#000000]   focus:border focus:border-[#27272A]">
    ...
    <CardContent class="text-white space-y-8 -mt-8  flex-1 nodrag nopan cursor-auto ">
      ...
      <div ref="searchQueryRef">
        ...
        <div>
        <div ref="embeddingRef" class="h-10 flex w-full flex-row items-center space-x-2">
          <p>Embedding</p>
          <NuxtIcon name="clarity:info-line" size="20" />
        </div>
        <p class="text-[#D1D5DB] text-sm">Required for text-to-vector conversion.</p>
       
     
      </div>
        
        <div>
        <div  ref="ingestDataRef"  class="h-10 flex flex-row w-full  items-center space-x-2">
          <p>Ingest Data </p>
          <NuxtIcon name="clarity:info-line" size="20" />
        </div>
