...
onMounted(async () => {
  ...
  if(embeddingRef.value && !data.saved) {
    ...
    addInputPort(id, data.embeddingVariable.id, 'yellow', embeddingRef.value.offsetTop + embeddingRef.value.clientHeight / 2-10)
  }
  if(ingestDataRef.value && !data.saved) {
    data.ingestDataVariable.id = nanoLowercaseAlphanumericId(10)
    addInputPort(id, data.ingestDataVariable.id, 'yellow', ingestDataRef.value.offsetTop + ingestDataRef.value.clientHeight / 2-10)
  }
