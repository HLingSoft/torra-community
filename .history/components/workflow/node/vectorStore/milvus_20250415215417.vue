...
onMounted(async () => {
  ...
  if(embeddingRef.value && !data.saved) {
    ...
    addInputPort(id, data.embeddingVariable.id, 'red', embeddingRef.value.offsetTop + embeddingRef.value.clientHeight / 2-16)
  }
  if(ingestDataRef.value && !data.saved) {
    data.ingestDataVariable.id = nanoLowercaseAlphanumericId(10)
    addInputPort(id, data.ingestDataVariable.id, 'red', ingestDataRef.value.offsetTop + ingestDataRef.value.clientHeight / 2-16)
  }
