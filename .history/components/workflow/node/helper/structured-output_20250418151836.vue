...
import { structuredOutputMeta } from '@/types/node-data/api-request'
...
onMounted(async () => {
    ...
    if (languageModelVariableRef.value && !node.data.saved) {
        ...
        addInputPort(props.id!, currentNode.value!.data!.languageModelVariable.id, 'aquamarine', languageModelVariableRef.value.offsetTop + languageModelVariableRef.value.clientHeight / 2)
    }
...
watch(() => currentNode.value?.data?.methodType, async (val, oldVal) => {
  ...
  if (val === 'get') {
    ...
    // ✅ 更新主 output port 位置
    await nextTick()
    if (structuredOutputVariableRef.value && currentNode.value.data!.dataOutputVariable.id) {
      const y = structuredOutputVariableRef.value.offsetTop + structuredOutputVariableRef.value.clientHeight / 2 + 4
    ...
    if (inputMessageVariableRef.value) {
      const id = nanoLowercaseAlphanumericId(10)
      currentNode.value.data!.bodyVariable.id = id
      addInputPort(nodeId, id, 'aquamarine', inputMessageVariableRef.value.offsetTop + inputMessageVariableRef.value.clientHeight / 2)
    }
...
<template>
    <Card v-if="currentNode && currentNode.data" class="!pb-0 w-96 text-white bg-[#18181B]  rounded-lg  flex flex-col focus:outline-none  focus:shadow-lg focus:shadow-[#000000]   focus:border focus:border-[#27272A]">
        ...
        <CardContent class="text-white space-y-8 -mt-8  flex-1 nodrag nopan cursor-auto ">
            ...
            <div v-if="currentNode.data.methodType!=='get'" ref="inputMessageVariableRef">
                <div class="flex flex-row items-center space-x-2">
